...
export default {
  ...
  methods: {
    ...
    listPages() {
      ...
      //   this.loading = false;
      // });
      this.$store.dispatch('website/getPageList')
      .then(({data}) => {
        console.log('获取到得页面数据，data：');
        console.log(data);
        this.pageList = data;
        this.loading = false;
      })
    ...
    addOrEditPage() {
      ...
      this.$store.dispatch('website/updatePageInfo',this.pageForum)
      .then(data => {
        console.log(data);
        if (data.flag){
          this.$notify.success({
    ...
    beforeUpload(file) {
      ...
      return new Promise(resolve => {
        if (file.size / 1024 /1024 < this.$global.UPLOAD_FILE_MAX_SIZE) {
          resolve(file);
        }
