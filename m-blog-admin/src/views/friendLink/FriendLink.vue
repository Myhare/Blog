...
export default {
  ...
  methods: {
    ...
    deleteLink(id) {
      var param = [];
      if (id == null) {
        param = this.linkIdList;
      } else {
        param = [id];
      }
      this.$store.dispatch('friendLink/deleteLink', param)
      .then(data => {
        if (data.flag) {
          this.$notify.success({
            title: "成功",
    ...
    addOrEditFriendLink() {
      ...
      this.$store.dispatch('friendLink/saveOrUpdateLink', this.linkForm)
      .then(data => {
        if (data.flag) {
          this.$notify.success({
            title: "成功",
    ...
    // 获取友链列表
    listLinks() {
      this.$store.dispatch('friendLink/getLinkList', {
        current: this.current,
        size: this.size,
        keywords: this.keywords
      }).then(({data}) => {
        // console.log('查询友链列表成功,data--->');
        // console.log(data);
        this.linkList = data.recordList;
